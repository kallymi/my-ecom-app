...
import { useCart } from "../../context/CartContext"; // <--- ON IMPORTE TON CONTEXT
import { 
  ShoppingBag, 
  User, 
  Truck, 
...
export default function Navbar() {
  ...
  const { totalItems } = useCart(); // <--- ON RÉCUPÈRE LA VRAIE VALEUR DYNAMIQUE
  ...
  return (
    <nav className={`sticky top-0 z-[100] transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <div className="flex items-center gap-2 md:gap-8">
          ...
          <Link to="/cart" className="relative group p-3 transition-transform active:scale-90">
            ...
            {/* On affiche le badge seulement si totalItems > 0 */}
            {totalItems > 0 && (
              <span className="absolute top-1.5 right-1.5 min-w-[18px] h-[18px] px-1 bg-indigo-600 text-white text-[9px] font-[1000] rounded-full flex items-center justify-center border-2 border-white animate-in zoom-in duration-300">
                {totalItems}
              </span>
            )}
          </Link>
          ...
          <div className="hidden md:block">
            {user ? (
              <div className="relative group">
                ...
                {/* ... Dropdown (identique au précédent) ... */}
                <div className="absolute right-0 mt-3 w-56 bg-white rounded-[2rem] shadow-2xl shadow-indigo-200/20 border border-gray-50 py-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 translate-y-2 group-hover:translate-y-0 overflow-hidden">
                    ...
                    <button onClick={logout} className="w-full flex items-center gap-3 px-6 py-3 text-rose-500 hover:bg-rose-50 transition-colors text-[10px] font-black uppercase tracking-widest">
      ...
      {/* --- MENU MOBILE DRAWER (Identique au précédent) --- */}
      {/* ... code du drawer mobile ... */}
      {/* MENU MOBILE (DRAWER LATÉRAL) */}
      <div className={`md:hidden fixed inset-0 z-[100] transition-all duration-500 ${
        ...
        {/* Contenu du Drawer */}
        <div className={`absolute right-0 top-0 h-full w-[280px] bg-white shadow-2xl transition-transform duration-500 ease-out flex flex-col ${
          ...
          <div className="p-8 flex flex-col gap-6">
            ...
            {/* On remplace Suivi par Mes Commandes */}
            <MobileLink 
              to="/my-orders" 
              label="Mes Commandes" 
          ...
          <div className="mt-auto p-8 border-t border-gray-50 bg-gray-50/50">
            {user ? (
              <div className="space-y-6">
                ...
                <button 
                  onClick={logout}
                  className="w-full py-4 bg-white border border-rose-100 text-rose-500 rounded-2xl text-[10px] font-black uppercase tracking-widest"
                >
                  Déconnexion
                </button>
              </div>
              ...
              <Link
                to="/login"
                className="block w-full py-5 bg-black text-white text-center rounded-2xl text-[10px] font-black uppercase tracking-widest"
              >
                Connexion
              </Link>
            )}
...
const MobileLink = ({ to, label, icon }) => (
  ...
  >
    <div className="flex items-center gap-3">
      {icon && icon}
      <span className="text-lg font-bold tracking-tight text-black group-hover:text-indigo-600 transition-colors">
        {label}
      </span>
    </div>
